...
def add_employee(filename, cur, conn):
    ...
    for employee in data:
        ...
        j_id = employee["job_id"]
        date = employee["hire_date"]
        salary = employee["salary"]
        cur.execute("INSERT OR IGNORE INTO employees (employee_id, first_name, last_name, job_id, hire_date, salary) VALUES (?,?,?,?,?,?)", (emp_id, fname, lname, j_id, date, salary))
...
def job_and_hire_date(cur, conn):
    ...
    for employee in results:
        year = int(employee[0][:4])
        month = int(employee[0][5:7])
        day = int(employee[0][8:])

        if ((year < earliest_y) or (year == earliest_y and month < earliest_m) or (year == earliest_y and month == earliest_m and day < earliest_d)):
            earliest_y = year
            earliest_m = month
            earliest_d = day
            job = employee[1]
...
def problematic_salary(cur, conn):
    out = []
    cur.execute("SELECT employees.first_name, employees.last_name FROM employees JOIN jobs ON employees.job_id = jobs.job_id WHERE salary < jobs.min_salary or salary > jobs.max_salary")
...
def visualization_salary_data(cur, conn):
    ...
    #plot max and min salary for each job title
